...
| B7 | **P1** | **Off-strip sport visible in strip slot 3** (swap like Create Game; last selected persists via `preferred_sports`) | `flow-play-screen` | `HomeScreen.tsx`, `DiscoverSportFilters.tsx` | **Shipped** — `fix/play-discover-tier2-ux` |
...
| B11 | **P1** | Discover empty-state hero icon — plain or 56px circle; no square tile | `module-sport-icon` | `DiscoverEmptyState.tsx`, `sportIconPresets.ts` | **Shipped** — `fix/play-discover-tier2-ux` |
